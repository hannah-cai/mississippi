...
function init() {
    ...
    var planeMaterial = getMaterial('standard', 'rgb(150, 150, 200)');
    var plane = getPlane(planeMaterial, 300, 50);
    plane.name = 'plane-1';
    ...
    // manipulate objects
    plane.rotation.x = Math.PI/2.02;

    var light = new THREE.AmbientLight( 0x0090FF); // soft white light
...
function onMouseWheel( event ) {
  ...
  camera.position.clampScalar( -500, 200 );
...
function update(renderer, scene, camera, clock) {
    ...
    planeGeo.vertices.forEach(function (vertex, index) {
        ...
        vertex.z += (noise.simplex2(z, z) + noise.simplex2(z+1, z))/2 * 0.05;
    });
